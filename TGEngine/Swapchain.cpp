...
void createSwapchain(Swapchain chain) {
	...
	if ((*chain.dev.khr_capabilities).minImageCount > chain.image_count) {
		swap_chain_creat_info.minImageCount = (*chain.dev.khr_capabilities).minImageCount;
		chain.image_count = swap_chain_creat_info.minImageCount;
	}
	else if ((*chain.dev.khr_capabilities).maxImageCount > chain.image_count) {
		swap_chain_creat_info.minImageCount = chain.image_count;
	}
	else {
		swap_chain_creat_info.minImageCount = (*chain.dev.khr_capabilities).maxImageCount;
		chain.image_count = swap_chain_creat_info.minImageCount;
	}
	swap_chain_creat_info.imageFormat = chain.dev.prefered_format;
	swap_chain_creat_info.imageColorSpace = chain.dev.color_space;
	swap_chain_creat_info.imageExtent = WINDOW::window.size;
	...
	handel(vkCreateSwapchainKHR(*chain.dev.device, &swap_chain_creat_info, nullptr, chain.swapchain));

	uint32_t image_count = 0;
	handel(vkGetSwapchainImagesKHR(*chain.dev.device, *chain.swapchain, &image_count, nullptr));
	vector<VkImage> image_array = {};
	image_array.resize(image_count);
	handel(vkGetSwapchainImagesKHR(*chain.dev.device, *chain.swapchain, &image_count, image_array.data()));
	...
	for (size_t d = 0; d < image_count; d++)
	{
		...
		imview_create_info.format = chain.dev.prefered_format;
		...
		handel(vkCreateImageView(*chain.dev.device, &imview_create_info, nullptr, &(chain.image_view_swapchain[d])));
	}
}

void destroySwapchain(Swapchain chain) {
	vkDestroySwapchainKHR(*(DEVICE::current_device.device),*chain.swapchain,nullptr);
}